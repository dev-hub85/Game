...
const SearchPage = () => {
  ...
  const searchQuery = decodeURIComponent(params.search as string);
  ...
  const filteredGames: Game[] = useMemo(() => {
    ...
    return allGames.filter((game) => {
      ...
      const translationMatch = game.translations?.en?.description
        ?.toLowerCase()
        .includes(query);

      return nameMatch || categoryMatch || descriptionMatch || translationMatch;
    });
      ...
      <section className="max-w-7xl mx-auto pt-5 px-2 md:px-4">
        ...
        <div className="flex flex-row md:flex-row gap-4 items-start md:items-center justify-between mb-6">
          ...
          <span className="text-white font-oxanium text-sm">
            {sortedGames.length} result{sortedGames.length !== 1 ? "s" : ""} for
            &quot;{searchQuery}&quot;
          </span>
        </div>
        ...
        <div className="flex flex-wrap -mx-2">
          {sortedGames.length === 0 ? (
            <div className="w-full px-2">
              <p className="text-[#b6c6e3] text-sm md:text-base">
                No games found for &quot;{searchQuery}&quot;. Try a different
                search term.
              </p>
            </div>
          ) : (
